...
describe('Full Contract Lifecycle E2E (Провайдер -> Договор -> Принятие)', () => {
  ...
  const login = (userLogin, userPassword) => {
    ...
    // После редиректа SPA ещё догружается (страница может быть пустой) —
    // дожидаемся отрисовки оболочки приложения (сайдбара), иначе следующий
    // клик по пункту меню не найдёт элемент.
    cy.get('.sidebar-link', { timeout: 40000 }).should('exist');
  };
  ...
  it('Создание провайдера (A) -> Создание договора (A) -> Принятие договора (B)', () => {
    ...
    // После выбора Чартера список провайдеров подгружается (GET /channels) и
    // первый провайдер выбирается автоматически. Список при этом перерисовывается,
    // поэтому НЕ кликаем по карточке (её элемент отваливается от DOM), а просто
    // ждём появления выбранной карточки — should('exist') безопасно
    // переспрашивает DOM даже при перерисовке.
    cy.get('.provider-card--active', { timeout: 20000 }).should('exist');
    ...
    // Ждём результаты поиска (список подгружается с сервера и может
    // перерисоваться) и кликаем на найденного партнёра. Разбиваем на отдельные
    // команды, чтобы клик безопасно переспрашивал DOM при перерисовке списка.
    cy.get('.partner-item', { timeout: 15000 }).should('have.length.at.least', 1);
    cy.get('.partner-item').first().click({ force: true });
    ...
    // При входе на шаг список провайдеров инвалидируется и повторно
    // подгружается — дожидаемся загрузки и даём ему устаканиться, чтобы
    // элементы не отвалились от DOM во время клика.
    cy.get('.product-group', { timeout: 15000 }).should('have.length.at.least', 1);
    cy.wait(1000);

    // Раскрываем все свёрнутые группы продуктов, чтобы увидеть провайдера
    cy.get('.product-group').each(($g) => {
      if (!$g.hasClass('product-group--expanded')) {
        cy.wrap($g).find('.product-group__header').click({ force: true });
      }
    });

    // Кликаем на провайдера, которого создали (avia). Разбиваем на assert + click,
    // чтобы клик безопасно переспрашивал DOM при возможной перерисовке списка.
    cy.contains('.provider-row__name', providerName, { timeout: 15000 }).should('exist');
    cy.contains('.provider-row__name', providerName).click({ force: true });
    ...
    // Список запросов подгружается с сервера — дожидаемся его и даём устаканиться.
    cy.log('⚠️ Открываем входящий договор');
    cy.get('.request-card', { timeout: 20000 }).should('have.length.at.least', 1);
    cy.wait(500);
    cy.get('.request-card')
      .first()
    ...
    // Список договоров подгружается с сервера — дожидаемся карточки и даём
    // устаканиться, затем разбиваем на assert + click для безопасной переспрашивки.
    cy.log('⚠️ Открываем созданный договор');
    cy.contains('.agreement-card', USER_B.login, { timeout: 20000 }).should('exist');
    cy.wait(500);
    cy.contains('.agreement-card', USER_B.login).click({ force: true });
