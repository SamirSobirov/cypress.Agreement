...
describe('Full Contract Lifecycle E2E (Провайдер -> Договор -> Принятие)', () => {
  ...
  const login = (userLogin, userPassword) => {
    ...
    cy.wait(1000);
    cy.get('button.sign-in-page__submit').should('be.visible').click({ force: true });
  ...
  it('Создание провайдера (A) -> Создание договора (A) -> Принятие договора (B)', () => {
    ...
    // Выбираем первого провайдера в списке (радио-карточка)
    ...
    // Финальное сохранение провайдера
    ...
    // Продолжить (Получатель -> Продукты)
    ...
    // Продолжить (Продукты -> Настройка договора)
    cy.get('.create-footer').contains('button', /Продолжить|Continue/i)
      .should('be.visible')
      .click({ force: true });
